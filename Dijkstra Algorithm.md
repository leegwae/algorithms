...
시작 정점 `S`에서 `B`로 가는 최단 경로는 `S - A - C - B`가 될 것이다. 그러나 BFS로 `S`부터 탐색한다면 `A`와 `B`를 먼저 방문한 후 `C`에 방문하게 된다. 최단 경로의 길이는 구할 수 있지만 경로가 어떤 정점으로 이루어지는지 알 수 없다. 다익스트라는 **우선순위 큐**를 사용하여 이 문제를 해결한다.
...
`16-17`: `distances[cur]`이 현재 경로의 길이 `cur_dist`보다 작다면 이미 시작 정점 `S`부터 `cur`까지의 최단 경로를 구하여 저장한 상태이므로 건너뛴다.
